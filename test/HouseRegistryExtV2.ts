...
describe('HouseRegistryExtV2', function () {
  ...
  it('should update contract and return houseId from first contract', async function () {
    ...
    await houseRegistryExtV2.connect(acc1).listHouseSimple(101, 101, 101, 'asd');
    const func = await houseRegistryExtV2.getExpensiveHouseIds();
    await expect(func).to.equal(houseId);
  });

   it('should return expensive houseId', async function () {
    await houseRegistryExtV2.connect(acc3).listHouseSimple(100, 100, 100, 'asd');
    const expensiveHouse = await houseRegistryExtV2.connect(acc1).listHouseSimple(200, 200, 200, 'asd');
